...
```
[가족 폰 — 앱 하나]                    [집 서버 / NAS]
카드사·결제 앱 알림/SMS 캡처 ─원문 전송─▶ 파싱 · 교차출처 대사 · 실적 계산
...
안드로이드 쪽은 PWA가 아니라 **네이티브 수집기 앱**입니다. 알림·SMS·오프라인 큐는
Android API로 구현하고, 화면만 Next.js 서버를 WebView로 표시합니다.
각 사용자는 앱 설정에서 설치된 카드사·결제 앱을 검색해 여러 개 등록하며, 네이티브
업데이트가 필요하면 같은 tailnet 서버의 APK를 받아 덮어쓰기 설치합니다.
...
Phase 2 진행 중입니다. 코드와 로컬 통합 검증은 끝났고, **앱 안의 실기기 수집 대상 등록·
개인정보 canary·실제 원문 수집이 남았습니다.** 현재 상태와 다음 할 일은
**[docs/HANDOFF.md](docs/HANDOFF.md)** 에서 확인하세요.

| Phase | 내용 | 상태 |
|---|---|---|
| 0 | 문서 · CI/CD 환경 | ✅ 완료 |
| 1 | 프로젝트 스캐폴딩 (Next.js · Prisma · Auth) | ✅ `v0.1.0` |
| 2 | 수집 파이프라인 (`/api/ingest` + 안드로이드 앱) | 🟡 코드 완료 · 실기기 검증 대기 |
| 3 | 파서 + 카드 매칭 | 대기 |
| 4 | 실적 엔진 | 대기 |
| 5 | 관리자 가족 대시보드 + 분석 | 대기 |
| 6 | 명세서 대사 · 운영 | 대기 |
...
- 같은 결제를 카드사 앱과 토스 등 여러 앱이 동시에 알릴 수 있습니다. 원문은 모두 보존하고
  실제 문구를 확보한 뒤 거래 한 건으로 대사합니다
...
- 각 사용자는 FamilyCard 설정에서 카드사·결제 앱, 카카오 공식 채널, SMS 발신자를 직접
  추가·삭제합니다. 목록이 비었거나 손상되면 아무 알림도 저장하지 않습니다
