...
아래는 기존 Phase별 버전 대응표입니다. 2026-09-25 사용자 지시에 따라 이번 연속 작업은
중간 변경을 Unreleased에 누적했고, 2026-09-26 사용자 배포 지시로 구현 완료 기능 전체를
0.3.0 검증용 업데이트에 합쳤습니다. 아래 표는 최초 계획이며 실제 Phase 완료를 뜻하지 않습니다.
...
- 0.3.0 운영 배포 결과와 남은 실기기/실사용 검증 조건 기록.

## [0.3.0] - 2026-09-26

실사용 검증용 배포. 앱 versionCode 8, 서버/앱 0.3.0. 사용자 최신 배포 지시에 따라
미확인 실기기·한 사이클 검증을 완료로 바꾸지 않고 기존 서명으로 업데이트합니다.
자동 파싱·알림은 검토한 규칙/설정 활성화 전까지 기본 비활성 상태입니다.

### Deployment

- PR #50 CI 후 0.3.0 운영 반영. 배포 직전 원문 994건 보존, HTTPS/DEVICE SELF/APK 검증 완료.

- 빈 INVITE_CODE가 Compose에서도 가입 닫힘으로 전달되도록 수정.
- 로컬에서 기존 키로 서명하고 해시를 고정한 APK를 CD가 검증·포함하는 배포 경로.


### Security

- R03: 초대 코드 미설정 시 가입 닫기, 웹 세션 버전 검증·관리자 철회·본인 비밀번호 변경.


### Fixed

- R01 보강: 수동으로 원거래를 지정한 취소는 해당 연결만 따라 후보 조회 비용 감소.

- R06: 서버 배치 상한의 200건 하한 보정으로 앱 큐 정체 방지, 카카오 제목 중복의 수집 한계 안내.

- R05: 대표 원문 분리 시 이동하는 근거 묶음을 확인 문구와 버튼에 명시.

- R04: 재처리의 일시 DB 충돌은 다음 주기에 재시도하고 연속 실패 상한·동시 진행 보호 적용.

- R01: 취소 투영을 변경 전후 거래의 의존 후보 창으로 제한하고 관리자 전체 비교/복구 도구 추가.
...
- 사용자 재연결 복구 확인과 현재 서명 키 PC/Google Drive 암호화 백업·복원 검증 기록.
  재부팅·복호화 암호 별도 보관은 미확인으로 유지하며 버전/게시 APK 변경 없음.

### Added
- 일/주/월 백업 보존 계획과 선택 독립 암호화 복사·복호화 검증·고정 백업 보호.
- 수집 상태 지연·실적 미달/구간 하락 알림 목록과 사용자 선택 일반 웹 푸시.
- CSV/XLSX 명세서 원본·행 보존, 컬럼 매핑·중복 방지·대사·검토 후 일괄 연결/보정.
- 가족×카드/구성원 분석·13개월 비교·예산 초과와 범위 제한 XLSX/PDF 보고서.
- 카드별 실적 추정치·기간/규칙 버전·제외 사유·취소 귀속 정책·산정 기록.
- 공용 카테고리 관리와 소유자별 가맹점 학습, 수동 실적 포함/제외 판단.
- 지속 범위 재처리·미리보기/악화 확인·대상 ID 고정·설정 변경 중단과 작업 진행 화면.
- 관리자 파싱 규칙 편집·샘플 시험·구조 초안·버전 이력·비활성 복원과 접근 통제.

- 카드/별칭 관리, 원문 보존 수동 거래·취소 연결·중복 병합/분리와 판단 기록.
- KST 월별 카드 거래·순사용액, 미확정/원화 미확정 분리와 scope 회귀 검증.

### Fixed
- legacy RCS TextView 취소 본문 추출과 실제 공백 구조 회귀, PDF standalone pnpm 경로 보강.
- 실적 조회를 해당 기간/연결 원거래로 제한하고 날짜 포맷터 재사용·256MiB V8/512MiB web 예산 검증.

- 동일 사건 번호인데 금액/가맹점이 다른 원문은 자동으로 새 확정 거래를 만들지 않고 검토.

### Added

- 원문과 처리 작업의 원자적 저장, 누락 작업 복구·lease/세대·재시도와 주기 처리기.
  복수 근거·취소의 트랜잭션 반영, 재처리 중 수동/기존 거래 보존. 운영 기본 비활성화.

- 제한된 RE2JS 규칙 실행과 RCS 설명 추출, 정수/외화·KST 날짜 검증 파서.
  구성원/기간/충돌을 고려한 카드 매칭과 보수적 복수 출처 대사·취소 원점 재계산 엔진.

- 거래별 복수 원문 근거, 규칙 버전/변경 snapshot, 처리 작업·검토 이력과 명세서 원본 모델.
  카드 끝번호 충돌/유효 기간, 외화 단위·원화 미확정·시각 정밀도, 기기 없는 원문의 소유 범위.
- 사용자 승인 ADR 0014에 따라 격리 개발과 최종 실기기 배포 검증을 분리. 중간 버전 발행 없음.

- private 일일 PostgreSQL 백업과 동시 실행/부분 파일 방어, 이전 성공 보존.
  자원 감시에 백업 실패·36시간 지연·파일 누락/크기 변경 경고 추가. 원문/기존 백업 자동 삭제 없음.

- 앱의 최신 버전 확인과 빌드 APK에 대응하는 버전·SHA-256 메타데이터 생성.
  리디렉션·잘못된 패키지·크기 초과 응답을 거부하며 최종 게시 시 APK와 함께 반영.

- 최종 APK 업데이트 사전 검사: 서명 검증·같은 패키지/인증서·버전 증가 확인.
  데이터 보존 업데이트와 Play Protect 경고별 대응·키 백업 미완료 조건 문서화.
...
- Android 확인 필요 원문의 사유·개별 상세·재전송. 같은 원문이 대기열에 보존된 뒤에만
  격리함에서 이동하며 실패·충돌은 롤백, 재거부는 다시 격리. 버전/게시 APK 변경 없음.

- 원문 목록에 서버 도착순 기본 정렬·수신순 선택, SMS/RCS 수집 방식 필터,
  수신/도착 시각과 전송 완료/분석 상태 구분 표시. 버전은 전체 작업 완료 후 한 번만 갱신.

- 승인된 전체 검토안을 S00~S10 단계별 실행 계획과 실기기 수집 검증표로 문서화.
  보안·기본 운영 선행, 지속 파싱·규칙 이력·새 문구 재처리·데이터 보존 검증 기준 보강.

- Android 선택형 **RCS 자동 보충**: 15분 주기, 하루 겹침 조회, 완료 구간 기록과 중단 후
  이어받기, 권한/미지원 상태·최근 확인 시각 표시. 기본값 꺼짐, APK versionCode 7.
- **등록 문자 발신자의 모든 문구 보관** 옵션: 사용자 확인 뒤 SMS/RCS의 거래 어휘 필터만
  해제해 새 형식을 보존. 광고·인증번호 포함 가능성 안내와 미등록 발신자 차단 유지.
- 초기 표본 이후 새 형식의 미확정 표시·규칙 추가·과거 재처리·멱등 검증을 Phase 3 계획에 명시.

- Android **과거 문자 가져오기** — 등록한 SMS 발신자의 최근 30/90/365일 수신 문자를
  사용자 실행·읽기 권한 허용 후 기존 큐로 전송. 실시간 수집과 같은 발신 시각 기반 ID,
  본문 접근 전 발신자 필터, 진행/결과 건수·중지·권한 오류 표시. APK versionCode 4.
...
- Next.js 15.5.24·sharp 0.35.4 및 Prisma/린트/테스트 전이 의존성의 보안 수정 버전 적용.
  전체·운영용 의존성 감사 경고 0건 확인, CI High/Critical 감사 관문 추가.
- 운영 백업의 접근 권한과 격리 복원 절차·검증 결과 기록. 원문 944건 보존 확인,
  새 이미지의 수집·세션·SELF 범위·기기 폐기 HTTP 검증 후 배포.
...
- Android 수동 전송이 기존 재시도 작업 뒤에 대기할 수 있던 예약 방식 보완.
  전송 건수·작업 상태 실시간 갱신, HTTP 번호/연결 오류 종류·시도 시각 표시,
  업로드 리디렉션 자동 추종 방지. 원문 큐 보존, APK versionCode 6.
  실기기 대기열 전송 복구와 SMS/RCS·부분취소 원문의 서버 도착 확인.
  이후 정상 연결에서 새 알림 자동 전송과 확인 필요(rejected) 0건을 사용자 확인으로 검증.
...
- 장기 수집용 기본 Compose를 Next.js standalone 운영 서버로 전환하고, 핫 리로드는
  ...
  standalone IPv4 바인딩과 일치하도록 Compose healthcheck 주소를 수정.
