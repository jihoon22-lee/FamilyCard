...
`jihoon22-lee/FamilyCard`가 **공개 저장소**입니다. 가족 금융 데이터를 다루는
프로젝트이므로 **private 전환을 권장**합니다.

지금은 문서만 있어 노출된 정보가 없지만, 앞으로 파서 픽스처·시드 데이터·
설정 예시를 다루게 되면 실수 한 번의 대가가 큽니다.
([불변 규칙 7](../AGENTS.md)이 이를 막기 위한 것이고 CI에도 검사를 넣었지만,
저장소가 private면 방어선이 하나 더 생깁니다.)

### 2. 기본 브랜치 정리 필요

빈 저장소에 첫 푸시가 들어가면서 `claude/family-card-expense-tracker-fwxj6m`가
**기본 브랜치**가 되었습니다. 그래서 머지 대상(base)이 없어 Phase 0의 PR을
만들지 못했습니다.

정리 방안:
```bash
# 현재 브랜치 내용으로 main 을 만들고 기본 브랜치로 지정
git push origin claude/family-card-expense-tracker-fwxj6m:main
# GitHub 저장소 설정에서 기본 브랜치를 main 으로 변경
```
이후 Phase 1부터는 작업 브랜치 → `main` PR 흐름으로 진행할 수 있습니다.
