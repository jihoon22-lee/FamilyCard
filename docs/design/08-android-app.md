...
설정 → 수집 상태 → **확인 필요 원문 보기**에서 20건씩 사유·수신/거부 시각을 확인합니다.
본문은 개별 상세를 열 때 한 건만 읽습니다. 재전송은 같은 사건 ID·원문·수신 시각으로
pending 저장을 검증한 뒤 rejected를 삭제하는 단일 트랜잭션입니다. 저장 실패나 다른
원문의 ID 충돌은 롤백합니다. 재거부는 다시 격리되며 예약 실패는 pending에 남아 자동
작업이나 지금 전송으로 재시도됩니다. 원문 수정/폐기 기능은 제공하지 않습니다.

SQLite 회귀 검증에는 테스트 전용 [Robolectric](https://robolectric.org/getting-started/)을
사용합니다. Android API 35에서 실제 QueueDatabase 메서드와 트랜잭션을 실행합니다.
...
- rejected `확인 필요` 건수
- 본문 없는 마지막 캡처/전송 상태
- 수동 즉시 전송: 기존 즉시 작업 체인을 대체하되 원문 큐 보존
- 수집 상태가 보일 때 건수/오류/시도 시각을 갱신하고 WorkManager 예약 상태 표시
