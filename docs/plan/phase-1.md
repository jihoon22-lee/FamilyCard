...
### 프로젝트 생성
- [ ] `web/` — Next.js 15 App Router + TypeScript (`strict: true`)
- [ ] Tailwind + shadcn/ui, **모바일 우선** 브레이크포인트
- [ ] ESLint + Prettier
- [ ] Vitest 설정 (Postgres 통합 테스트 포함)
- [ ] `web/Dockerfile` — 멀티스테이지(`dev` / `prod` 타깃)

### 데이터베이스
- [ ] `prisma/schema.prisma` — [01-data-model](../design/01-data-model.md)의 전체 스키마
  - `FamilyMember`(role 포함) · `Device` · `Card` · `CardAlias`
  - `RawMessage` · `ParserRule` · `Transaction`
  - `Category` · `MerchantRule` · `CardBenefitRule` · `Budget`
- [ ] 인덱스 — 특히 `RawMessage.dedupeHash` UNIQUE, `Transaction(cardId, amount, approvedAt)`
- [ ] 초기 마이그레이션
- [ ] `prisma/seed.ts` — 개발용 가족 2명(ADMIN 1, MEMBER 1), 카드 몇 장, 기본 카테고리

### 인증 · 가시성 ★
- [ ] Auth.js Credentials
- [ ] 초대 코드(`INVITE_CODE`) 기반 가입
- [ ] 세션에 `memberId` · `role` · `scope` 탑재
...
- [ ] `src/app/(family)/page.tsx` — 빈 가족 화면 (ADMIN 전용)
- [ ] 로그인 · 로그아웃
