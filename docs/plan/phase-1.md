...
> 버전 태그: `v0.1.0`
>
> **진행 상황**: Phase 1 완료 — W1 ✅ (PR #1) · W2 ✅ (PR #2) · W3 ✅ (인증 · scope · UI 셸, CI 실효성 확인 완료)
> 다음 작업은 [HANDOFF](../HANDOFF.md)와 [Phase 2](phase-2.md) 참고.
...
### 검증
- [x] CI가 `web/` 코드를 실제로 검증하는지 확인 (일부러 타입 오류를 넣어 빨간불이 뜨는지) —
  `web/src/lib/auth/scope.ts`에 의도적 타입 오류 삽입(`error TS2322`) → `web`·`ci-ok` 잡 실패 확인 → 원복 후 초록 복귀
...
- [x] 리뷰 R03: 가입 닫기·웹 세션 철회·본인 비밀번호 변경·권한/격리 migration 검증 (#45).
