...
- [x] **가족 × 카드 매트릭스** — 행: 구성원, 열: 카드, 값: 사용액 + 실적 달성률
- [x] 구성원별 합계 · 카드별 합계 · 가족 총액
- [x] 구성원 드릴다운 (`/family/members/[id]`)
  - **`visibleMemberIds()`에 포함되는지 확인 후 조회** ★
- [x] 실적 미달 임박 알림 조건·취소 소급 변화 알림 (S09에서 연결; 부족액 표시는 구현)
- [x] 미확정 큐 적체 현황 — 구성원별로 몇 건이 방치 중인지
- [x] 수집기 무응답 기기 경고

### 카테고리 분류
- [x] `MerchantRule` 기반 소유자별 정확 가맹점 학습 (모호한 중복은 미확정 유지)
- [x] 기본 카테고리 세트 시드 (식비 · 교통 · 쇼핑 · 공과금 · 의료 · 문화 · 기타)
- [x] 미분류 건에 사용자가 카테고리 지정 → **`MerchantRule` 자동 생성**
- [x] 카테고리 관리 화면

### 분석
- [x] 전월 대비 · 전년 동월 대비 추이 차트
- [x] 카테고리별 비중 (구성원별 / 가족 전체)
- [x] 예산 설정 (`Budget` — 구성원/카테고리 조합)
- [x] 예산 초과 경고
...
- [x] MEMBER 웹 로그인 → 본인 것만
- [x] MEMBER 세션으로 `/family` 접근 → 리다이렉트 또는 403
- [x] **MEMBER 세션으로 API에 타인 `memberId` 직접 전달 → 403 또는 빈 결과**
- [x] ADMIN 웹 로그인 → 가족 전원
- [x] **ADMIN 폰 앱 → 본인 것만** (scope가 진입 경로로 결정되는지)
- [x] ADMIN 디바이스 토큰으로 세션 발급 → `scope: SELF`
- [x] 만료된 nonce / 재사용 nonce → 401
- [x] 폐기된 디바이스 토큰 → 401
...
구현/가공 회귀 검증 상태입니다. 가족 전원의 실사용·권한 실기기 확인은 최종 배포 검증에 별도 기록합니다.
