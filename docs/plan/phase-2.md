...
> 버전 태그: `v0.2.0`
>
> **진행 상태 (2026-09-25)**: 서버·Android·운영 배포 경로와 사용자 관리형 수집 대상의
> 코드·자동 검증 완료. v6 실기기 대기 0건, SMS/RCS를 포함한 대기열 전송 복구 확인.
> 이후 사용자 확인으로 rejected 0건과 수동 전송 없이 새 알림 도착도 확인했습니다.
> **Phase 자체는 미완료**입니다. 실기기 개인정보 canary·자동 복구·실제 결제 문구 분류·
> 서명 키·가족 전체의 수집 확인이 남았습니다.
...
실기기 결과는 [수집 검증표](collection-validation.md), 선행 운영 작업은
[단계별 계획 S01/S02](staged-execution-plan.md)에 연결합니다.

- [x] 전체 단계 실행 계획·신규 보강·실기기 검증 근거 표 문서화
- [x] S02-A 보안 의존성 점검·수정·검증·배포 — 전체/운영용 감사 경고 0건, Web 150 tests
- [x] S02-B 현재 운영 백업·격리 복원과 보존 증거 확보 — 기준 원문 944건 누락/변경 0

- [x] 실기기 원문이 서버에 쌓임 — 2026-09-24 읽기 전용 집계 확인 (거래성 문구 분류는 대기)
- [x] 현재 기기에서 정상 연결 중 새 알림 자동 전송·확인 필요 0건 — 2026-09-25 사용자 확인
- [x] 네트워크 재연결 후 전송 복구 — 2026-09-25 사용자 확인; 기내모드 사용·출처별 범위는 미기록
- [x] 현재 게시 APK 서명 키의 PC·Google Drive 암호화 복사와 재다운로드·복호화·원본 일치 검증
- [ ] 복호화 암호의 PC 밖 별도 보관 확인
...
### 삼성 RCS 가져오기 (2026-09-24)

- [x] 등록 발신자·수신 종류·기간을 본문 조회 전에 검사하는 삼성 호환 경로
- [x] JSON 원문/RCS 출처 보존과 재실행 ID, 긴 본문 요청 분할
- [x] 실기기 삼성 RCS 조회·취소 원문 서버 도착 확인 (v6 전송 복구 후)
- [ ] 동일 기간 RCS 재가져오기 시 중복 없음 확인
- [x] 사용자 선택 RCS 자동 보충 — 15분 주기, 하루 겹침, 완료 구간 기록·중단 후 재개
- [x] 미지원/권한 거부/실패에서 완료 구간 유지와 상태 표시
- [ ] v7 실기기 자동 RCS 신규 도착·끄기·재부팅·오프라인 복구 검증
- 실시간 RCS 콜백은 지원하지 않음. 자동 보충은 OS에 의해 지연될 수 있음.

### 새로운 문자 형식 보존 (2026-09-24)

- [x] 등록 발신자의 모든 SMS/RCS 문구 보관 옵션과 광고·인증번호 포함 확인창
- [x] 기본 거래 어휘 필터 유지, 모든 문구 모드에서도 미등록 발신자 본문 비접근
- [x] 작업 시작 후 범위 확대 금지·설정 철회 반영·동일 사건 ID 유지
- [ ] 실기기 모든 문구 선택/철회와 미등록 개인 문자 비수집 확인

### 과거 SMS 가져오기 (2026-09-24)

- [x] 사용자 실행 + 선택 기간(30/90/365일) + READ_SMS 런타임 권한
- [x] 등록 발신자 본문만 읽고 기존 거래 어휘/큐/서버 멱등 경로 사용
- [x] DATE_SENT 없는 항목의 보류, 중지·부분 실패 상태와 원문 보존
- [x] 실기기 과거 SMS 조회·서버 도착 확인 (v6 전송 복구 후)
- [ ] 동일 기간 SMS 재가져오기 시 중복 없음 확인

### Android 큐·업로드

- [x] 수동 재시도가 기존 즉시 작업 체인을 대체하고 예약 완료 확인
- [x] 건수/전송 상태 자동 갱신, HTTP·연결 오류 분류, 작업 실행 상태 표시
- [x] v6 실기기 대기 0건·전송 완료와 SMS/RCS 서버 도착 검증
  - 이전 실패의 정확한 HTTP 오류 번호는 미확인.
- [x] 정상 연결에서 지금 전송 없이 새 알림 도착·확인 필요 0건 (사용자 확인 + 서버 신규 도착 집계)
  - 재연결 복구는 사용자 확인. RCS 자동 보충·재부팅 뒤 복구·장기 지속성은 별도 실기기 검증 필요.
...
- [x] 장기 수집 기본 Compose를 standalone으로 전환, Docker 개발 설정 분리
- [x] 운영 Prisma 클라이언트/연결 풀 재사용 회귀 테스트
...
- [x] 20건 단위 사유 목록·개별 원문 상세·개별 재전송 구현
- [x] 대기열 저장 확인 후 격리함 이동, 실패/충돌 롤백·재거부 보존 구현
- [ ] 최종 APK 설치 뒤 실기기에서 격리함 동작 확인 (현재 사용자 격리 건수는 0)
...
- [x] APK 실제 서명/패키지/버전 비교 도구와 가공 회귀 테스트
- [x] 현재 게시·후보 APK의 동일 서명/패키지, 버전 7 유지 확인 (게시하지 않음)
- [x] [업데이트 보존·Play Protect 검토](../guide/safe-final-update.md) 문서화
- [ ] 운영 서명키의 독립된 암호화 백업 두 곳·전환 경로·실기기 보존 검증

- [x] 동일 APK의 버전/해시 메타데이터 생성·앱 최신 버전 확인 구현 (게시 대기)

## 독립 운영 작업 선행

- [x] private 일일 백업·동시 실행 잠금·실패 시 이전 성공 보존·36시간 지연 감시
- [x] 새 자동 백업의 격리 DB 복원·기준 원문 946건 누락/변경 0
- [ ] 외부 백업 사본·보존 기간 정책 확정

- [x] 리뷰 R06: 서버 배치 최소 200건·설정 경계 회귀, 카카오 동일명 개인 대화 경고/검증표 추가 (#48).
