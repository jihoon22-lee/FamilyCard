...
이번 0.3.0 후보는 versionCode 8이며, 업데이트 비교 기준은 기존 code 7입니다. 새 기능의 빌드 성공은 실기기 배포 완료가 아닙니다.
...
2026-09-25 사용자가 PC + Google Drive를 백업 장소로 지정하고 진행을 승인했습니다.
현재 게시 APK와 로컬 개발 서명 키의 인증서 일치를 확인한 뒤 AES256으로 암호화했습니다.
이는 **현재 설치를 유지하는 개발 서명 키 보존**이며 새 운영 서명 키 발급/전환이 아닙니다.

- PC: `~/FamilyCard-Backups/familycard-signing-20260925.tar.gz.gpg` (동기화 폴더 밖, 접근 제한).
- Google Drive: 사용자 소유 비공유 `FamilyCard Backups` 폴더에 같은 파일을 저장했습니다.
- Drive에서 재다운로드한 파일의 SHA-256, 복호화 성공, 복원한 키와 원본의 바이트 일치를 확인했습니다.
- 백업에는 현재 서명 키와 복구용 manifest만 포함합니다. 금융 원문/DB/환경설정은 포함하지 않습니다.
- 복호화 암호는 private `data/secrets/backup-passphrase`에 있고 Drive에 올리지 않았습니다.
  사용자가 비밀번호 관리자 등 **PC 밖 별도 장소**에 보관했다는 확인은 아직 없습니다.
- 위치/검증 상세는 private `data/verification/signing-backup.json`에 기록합니다.
  키·암호는 채팅/로그/Git에 남기지 않습니다. GitHub Secrets도 변경하지 않았습니다.

PC를 잃어도 복구하려면 암호의 별도 보관까지 끝나야 합니다. 따라서 S02-E 운영 키 전환과
독립 복구 준비 전체를 완료로 표시하지 않습니다. 재부팅 검사는 사용자 요청으로 추후 진행합니다.
...
## 0.3.0 검증용 배포 (2026-09-26)

최신 사용자 지시에 따라 [ADR 0024](../adr/0024-user-authorized-validation-release.md)로 이번 배포 대기를 변경했습니다.
미확인 실기기/정확도 조건은 후속 검증으로 유지합니다. 기존 인증서로 서명한 release APK의
패키지/인증서 일치와 code 7→8 상승을 검사하고, 디버그 가능 플래그가 없는지 확인합니다.

개인 키 없이 CD를 실행하려면 로컬 빌드 APK와 familycard.json을 병합 커밋 대상 draft
Release v0.3.0에 먼저 올립니다. `android/release-artifact.json`에는 동일 APK의 공개 SHA-256,
인증서, 패키지, 버전을 고정합니다. 태그 push 후 CD는 APK를 다시 내려받아 검증하고
동일 APK를 GHCR 서버 이미지/Release에 포함합니다. APK 이름이 이미 familycard.apk인
경우 자체 경로로 mv하지 않습니다. 키/암호를 GitHub Secrets에 등록할 필요가 없습니다.

INVITE_CODE를 비우면 개발/운영 Compose 모두 빈 값을 전달해 가입이 닫힙니다.
이번 배포는 현재 초대 코드나 실제 파서/상품 조건을 임의 변경하지 않습니다.
