...
> **현재 배포 상태**: 사용자 지시에 따라 0.3.0 검증용 업데이트를 운영 반영했습니다.
> 새 서버/앱의 기능은 사용할 수 있지만 실제 카드/규칙 설정과 실기기·한 사이클 대조는
> 미완료입니다. 자동 처리·외부 푸시는 기본 비활성입니다. [ADR 0024](../adr/0024-user-authorized-validation-release.md).

---

일회성 운영 백업·격리 복원과 최근 검증 결과는 [백업·복원 가이드](backup-restore.md),
의존성 감사 명령과 수정 근거는 [보안 기준선](../research/security-baseline-2026-09-25.md)을
참고합니다. 정기 백업과 장기 감시는 단계별 계획에서 별도로 추적합니다.
...
태그를 push하면 CD가 릴리스 APK를 검증합니다. signing secret이 없으면 로컬 서명 APK를
draft Release에서 내려받아 고정 manifest와 비교합니다. 자세한 순서는 [서명 보존 가이드](safe-final-update.md)를 따릅니다.
...
아래는 개발 전용 debug APK 게시 명령입니다. 이번 0.3.0은 로컬 release APK를 검증해 게시했으므로 이 명령으로 덮어쓰지 않습니다. 생성된 APK는
Git에 커밋되지 않습니다.
...
1. 카드 관리에서 카드와 결제일을 확인하고 실적 규칙 설정으로 이동합니다.
2. 공식 상품 안내/약관 주소, 적용 시작일, 구간, 제외 항목과 취소 차감 정책을 입력합니다.
   입력 예시는 실제 상품의 조건이 아니므로 반드시 수정·확인합니다.
3. 혜택 판정월과 표시된 실제 산정 기간을 확인한 뒤 카드사 앱 수치와 대조합니다.
4. 차이가 나면 거래별 포함/제외·미확정·원거래 없는 취소를 확인합니다. 거래 목록에서
   분류나 승인 원거래의 수동 포함 판단을 고칠 수 있습니다. 가맹점 학습은 해당 구성원만 적용합니다.
5. 대조한 추정치를 기록하면 당시 기간·규칙 버전·거래별 기여액이 보존됩니다. 규칙 변경은
   새 버전이며 실제 원문과 이전 기록은 그대로 남습니다. 한 사이클 대조 전에는 정확도 완료로 보지 않습니다.


## 운영 알림과 선택 웹 푸시

최종 배포 시 `FAMILYCARD_ALERTS_ENABLED=true`를 설정하면 한 시간 주기로 알림 목록을
갱신합니다. 푸시 없이 목록만 사용할 수 있습니다. 푸시는 `scripts/generate-web-push-keys.mjs`로
한 번 생성한 private `data/secrets/web-push.env`의 세 변수를 배포 환경에 설정합니다.
키를 다시 만들면 기존 브라우저 구독이 무효화될 수 있으므로 암호화 백업을 보관하세요.

사용자는 `/alerts`에서 브라우저 권한을 허용하고 수신을 직접 켭니다. 외부 푸시 서비스로는
“확인할 알림이 있습니다”만 전달합니다. 가족 알림 수신은 관리자 웹만 선택할 수 있습니다.
5회 연속 실패 후 전달을 멈추며 다시 수신을 켜면 재시도합니다. 404/410 구독은 꺼집니다.
원문/금액/엔드포인트/키를 로그나 지원 메시지로 보내지 않습니다. 실제 수신은 최종 배포 후 확인합니다.
...
관리자 웹의 `/family/projection`에서 소유자·카드 이름으로 카드를 선택하고 전체 취소 반영 차이를 비교합니다. 기본은 드라이런이며 명시적으로 반영을 선택해야 저장합니다. 처리 중 PROJECTION_WINDOW_LIMIT가 나타나면 연쇄 후보가 10,000건을 넘은 것입니다. 이 도구는 100,000건까지 지원하고 반영 시 감사 기록을 남깁니다. 정상 수집에서 전체 재계산은 실행하지 않습니다.

## 가입 닫기와 웹 세션 종료

가족 등록을 마치면 운영 환경의 INVITE_CODE를 비우고 최종 배포 절차에 따라 서버를 재생성합니다. 빈 값/미설정은 가입 닫힘이며 기존 로그인은 유지됩니다. `/family/sessions`에서 구성원의 모든 웹 세션을 종료할 수 있습니다. `/account`에서는 현재 비밀번호를 확인하고 본인 비밀번호를 변경합니다. 변경 뒤 모든 웹 세션에서 다시 로그인해야 합니다. 앱 DEVICE 세션·수집 토큰에는 영향을 주지 않습니다. sessionVersion 도입 전 웹 쿠키도 다시 로그인해야 합니다. 역할이 변경된 웹 쿠키는 다음 요청에서 거부됩니다. 0.3.0 운영 서버에 반영되었습니다.

## 수집 배치 설정

앱의 배치는 200건입니다. INGEST_MAX_BATCH_SIZE가 200 미만이거나 잘못된 정수이면 서버는 200으로 보정합니다. 200 이상 정수 설정은 유지합니다. 요청 바이트 상한 INGEST_MAX_REQUEST_BYTES는 별도이므로 기본 6,000,000바이트를 임의로 줄이면 정상 배치가 413으로 거부될 수 있습니다.
