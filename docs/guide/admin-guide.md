...
가족 등록을 마치면 운영 환경의 INVITE_CODE를 비우고 최종 배포 절차에 따라 서버를 재생성합니다. 빈 값/미설정은 가입 닫힘이며 기존 로그인은 유지됩니다. `/family/sessions`에서 구성원의 모든 웹 세션을 종료할 수 있습니다. `/account`에서는 현재 비밀번호를 확인하고 본인 비밀번호를 변경합니다. 변경 뒤 모든 웹 세션에서 다시 로그인해야 합니다. 앱 DEVICE 세션·수집 토큰에는 영향을 주지 않습니다. sessionVersion 도입 전 웹 쿠키도 다시 로그인해야 합니다. 역할이 변경된 웹 쿠키는 다음 요청에서 거부됩니다. 이번 변경은 아직 운영에 배포하지 않았습니다.
