...
**기본 원칙: Phase 2(수집)를 완주하기 전에 Phase 3(파서)를 시작하지 않습니다.**

**2026-09-25 사용자 승인 예외:** [ADR 0014](docs/adr/0014-development-and-release-gates.md)에
따라 수집 구현·원문 보존·격리 복원을 개발 기준선으로 S03~S09를 기능 브랜치와 격리 DB에서
진행합니다. 개인정보/RCS/복구/서명/가족 커버리지의 실기기 검증은 최종 배포 조건으로 유지하며,
미확인을 성공으로 표시하지 않습니다. 실제 표본 없는 규칙은 추측해서 활성화하지 않습니다.
버전·태그·게시 APK는 전체 작업 종료 후 한 번만 갱신하고 병합한 브랜치/임시 워크트리는 정리합니다.

**2026-09-26 최신 배포 지시:** [ADR 0024](docs/adr/0024-user-authorized-validation-release.md)에 따라
0.3.0 실사용 검증용 업데이트를 배포합니다. 미확인 실기기/한 사이클 검증은 후속 조건으로
남기며 원문 보존·권한·동일 서명 업데이트 원칙은 유지합니다.
